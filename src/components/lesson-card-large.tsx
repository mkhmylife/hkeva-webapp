...
export default function LessonCardLarge(props: Props) {
    ...
    <Card>
      ...
      <div className="py-4">
        <div className="mb-1 flex justify-between items-center gap-1">
          <h1 className="font-semibold text-xl">{lesson.course?.name}</h1>
          <div className="text-brand-neutral-500 whitespace-pre">
            {lesson.course?.category2?.name}
            {lesson.course && lesson.course.lessons && lesson.course.lessons.length > 0 ? (
              <>
                {" · "}{lesson.course.lessons.length}節
              </>
            ) : null}
          </div>
        </div>
        <p className="text-sm tracking-wider font-medium text-brand-neutral-500">{lesson.course?.code}</p>
        ...
        <div className="space-y-2 mt-3">
          <div className="flex items-center gap-2">
            ...
            <p className="text">{lesson.room?.name}</p>
          </div>
          <div className="flex items-center gap-2">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
